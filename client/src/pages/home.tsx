...
export default function Home() {
  const { t } = useLanguage();
  const { toast, ToastComponent } = useToast(); // ✅ grab both
  ...
  // 🌧 Fetch rain alert when component loads
  useEffect(() => {
    const fetchRainAlert = async () => {
      try {
        const { data } = await axios.get("/api/weather/rain-alert?city=Delhi");
        if (data.rainExpected) {
          setRainAlert(true);
          toast({
            title: "🌧 Rain Alert",
            description:
              "Rain is expected soon in your area. Plan farming accordingly!",
            variant: "destructive", // red highlight
          });
        }
      } catch (err) {
        console.error("Rain alert fetch failed:", err);
      }
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {farmProfile ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="font-medium" data-testid="text-previous-crop">
                    {farmProfile.previousCrops?.length
                      ? farmProfile.previousCrops[0]
                      : "None"}
                  </p>
                </div>
              ...
              <div className="text-center py-4">
                <p
                  ...
                  data-testid="text-no-profile"
                >
                  No farm profile found. Create one to get personalized
                  recommendations.
                </p>
                <Link href="/profile">
        ...
        <Card className="shadow-sm">
          <CardContent className="p-6">
            <h2
              className="text-lg font-semibold text-card-foreground mb-4"
              data-testid="text-recommendations"
            >
            ...
            <div className="space-y-3">
              <div
                className="flex items-center justify-between p-3 bg-primary/5 rounded-md border border-primary/20"
              ...
              <div
                className="flex items-center justify-between p-3 bg-muted/50 rounded-md border border-border"
                data-testid="card-recommendation-3"
              >
                <div className="flex items-center space-x-3">
